...
import { traerCantidad, traerCantidadPorSede } from '../../services/servicesInscriptos'
...
const InscriptosPage = () => {
  ...
  useEffect(()=>{
    ...
    const getTraerDatos = async ()=>{
      if(anio>2016){
          setCantidad(await traerCantidad(anio))
          setCantidadSede(await traerCantidadPorSede(anio))

    }else{
      setCantidad(await traerCantidad(1))
      setCantidadSede(await traerCantidadPorSede(1))
    }
    }
...
*/
  ...
  return (
    ...
    <div className="row">
     {cantidadSede?
     <InscriptosAnioSede cantidad={cantidad} cantidadSede={cantidadSede} anioT={anioTitulo}  />
    :null 
    }
    </div>
     </div>
